...
const RootComponent = (props) => {
  ...
  const addProduct = (product) => {
    ...
    const newAddedProducts = cart.products.map((prod) => {
      if (prod.id === productId)
        return {
          ...prod,
          qty: ++prod.qty,
          price: productPrice * prod.qty,
        };
      return prod;
    });

    const newAddedTotal = newAddedProducts.reduce(
      (accumulator, current) => accumulator + current.price,
      0
    );

    setCart({ ...cart, products: newAddedProducts, totalPrice: newAddedTotal });
  };
  ...
  const removeProduct = (product) => {
    ...
    const newRemovedProducts = cart.products.map((prod) => {
      const quantity = prod.qty - 1 < 0 ? 0 : prod.qty - 1;
    ...
    setCart({
      ...
      totalPrice: newRemovedTotal(newRemovedProducts),
    });
  };
